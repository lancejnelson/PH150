...
#    axes.plot(t2,y2,'b--')
    ...
    axes.text(0.85, 1,f"slope = {m:5.2f}",fontdict = font)
    axes.set_xlabel("t")
    axes.set_ylabel("y(t)")
...
The notation ${dy(t) \over dt}$ is used to indicate the sloping function corresponding to $y(t)$.  ($y'(t)$ is another way to indicate this function.)
...
When the function of interest has more than one independent variable, derivatives are performed on each variable separately.  The product ${\partial f \over \partial x_i} \delta x_i$ is formed and inserted into the equation below 
